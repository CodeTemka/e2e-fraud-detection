...
def test_select_and_register_promotes_custom(monkeypatch):
    ...
    monkeypatch.setattr(model_competition, "_find_existing_model", lambda *_args, **_kwargs: None)
...
def test_select_and_register_promotes_best_when_no_prod_metric(monkeypatch):
    captured = {}
    ...
    def fake_best_run_by_metric(*, metric, direction, settings, experiment_name, status=None):
        if experiment_name == "automl-exp":
            return BestRun(experiment_name=experiment_name, run_id="auto-1", metric_name=metric, metric_value=0.72)
        return BestRun(experiment_name=experiment_name, run_id="custom-1", metric_name=metric, metric_value=0.75)

    def fake_register_custom_model_from_run(*_args, **kwargs):
        captured["custom"] = kwargs
        return SimpleNamespace(name="production-model", version="11")

    monkeypatch.setattr(model_competition, "best_run_by_metric", fake_best_run_by_metric)
    monkeypatch.setattr(model_competition, "_resolve_prod_metric", lambda *_args, **_kwargs: None)
    monkeypatch.setattr(model_competition, "_find_existing_model", lambda *_args, **_kwargs: None)
    monkeypatch.setattr(model_competition, "register_custom_model_from_run", fake_register_custom_model_from_run)
    monkeypatch.setattr(
        model_competition, "register_model_from_run", lambda *_args, **_kwargs: None
    )
    monkeypatch.setattr(model_competition, "mlflow_tracking_uri", lambda *_: None)
    ...
    assert result.promoted is True
    assert result.model_source == "custom"
    assert result.model_version == "11"
    assert captured["custom"]["run_id"] == "custom-1"
